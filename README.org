...
~outline-stars~ brings ~org~-like section folding to non-~org~ buffers using star-based comment headings (~;;; *~, ~### *~, ~// *~, /etc./) and the built-in ~outline-minor-mode~.  It handles comment prefix detection, per-level fontification, ~imenu~ integration, automatic numbering, and star-aware structure editing in a single, dependency-free file.
...
| ~outline-stars-number-region~     | Number headings in the active region (inherits)      |
